...
async function generateLive(
  ...
  const visitorMap: Partial<Record<TargetLanguage, new () => any>> = {
    ...
    csharp: CodeGen.CSharpVisitor,
    rust: CodeGen.RustVisitor,
...
export async function generate(
  ...
  try {
    ...
    if (fallback) {
      return { output: fallback, isLive: false, error: errMsg };
    }
    // No static fallback — surface the error so the UI shows it clearly
    return { output: "", isLive: false, error: errMsg };
  }
}
